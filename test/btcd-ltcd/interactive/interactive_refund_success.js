...
/** Run an interactive refund success scenario.
  ...
  This btcd-ltcd can run autonomously on regtest or interactively on testnet.
  ...
  This btcd-ltcd is interactive on the Lightning invoice and the swap address. It
...
module.exports = (args, cbk) => {
  return asyncAuto({
    // Determine which network to run the btcd-ltcd against
    promptForNetwork: cbk => promptForInput({
      default_value: 'regtest',
      explain: 'Type "testnet" to run btcd-ltcd on testnet instead of regtest',
      role: 'TEST',
    },
    cbk),
