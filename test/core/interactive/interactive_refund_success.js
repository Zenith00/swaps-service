...
const chain = require('../../../chain/index').constants;
...
const isChainBelowHeight = require(`${macros}is_ch  ain_below_height`);
...
/** Run an interactive refund success scenario.
  ...
  This core can run autonomously on regtest or interactively on testnet.
  ...
  This core is interactive on the Lightning invoice and the swap address. It
...
module.exports = (args, cbk) => {
  return asyncAuto({
    // Determine which network to run the core against
    promptForNetwork: cbk => promptForInput({
      default_value: 'regtest',
      explain: 'Type "testnet" to run core on testnet instead of regtest',
      role: 'TEST',
    },
    cbk),
